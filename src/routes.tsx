...
const routes: Route[] = [
    ...
    {
        element: <ProductPage />, path: '/Products', menuLabel: 'Produkter', loader: async () => {
            ...
            return { liftDetails, fuels, liftCategories };
        }
    },
    {
        element: <BookingPage />, path: '/booking', menuLabel: 'Boka',
        ...
        children: [
            {
                ...
                loader: async () => {
                    ...
                    const orders = await (await fetch("/api/orders")).json();
                    const orderItems = await (await fetch("/api/orderItems")).json();
                    return { lifts, users, orders, orderItems };
                }
            },
            {
                path: "orders",
    ...
    },
    {
        element: <ProductDetailsPage />, path: '/products/:id', loader: async ({ params }: { params: { id: string } }) => {
            const response = await fetch(`/api/liftDetails/${params.id}`); return response.json();
        }
    },
